...
struct Option
{
    std::string input_filename = "datasetB2_1.txt";
    std::string endfile_dir = "encrypteddatasets";
};
...
void create_querier_server(opdo::CallbackParam& param,
                           ...
                           const char* port = DATAB_PORT_FOR_QUERIER)
{
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        STDSC_LOG_INFO("Launched dataownerB demo app");
        stdsc::StateContext state(std::make_shared<opdo::StateInit>());
        opdo::CloudClient client(opsh::kDataOwnerB, "localhost",
                                 CLOUD_PORT_FOR_DATAOWB, state);
        opdo::CallbackParam param(client);
        param.input_filename = option.input_filename;
        param.encfile_dir = option.endfile_dir;
